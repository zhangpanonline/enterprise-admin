...
export default {
  ...
  setup() {
    ...
    return { appStore, historyRef, downloadDistRef, popoverRef, getObsUrl }
  },
  data() {
    return {
      ALL,
  ...
  methods: {
    async getOne() {
      try {
        const res = await getbankruptManagerOneApi(this.case.projectId)
        if (res.success) {
          if (res.data) {
    ...
    onDlownloadDist() {
      ...
      this.downloadDistRef.getTree(this.case.projectId)
    },
    clickPopoverOut(e) {
    ...
    async getTree() {
      try {
        const res = await findFolderTreeApi({ projectId: this.case.projectId })
    ...
    ondeExpand(file) {
      if (file.fileType === 0) {
        // 管理人团队、债务人可以访问债务人资料
        if ([MANAGE, DEBTOR].includes(this.role.org) || this.isLaw) {
        ...
        // 管理人团队、监督机构可以访问监管资料
        if ([MANAGE, SUPERVISE].includes(this.role.org) || this.isLaw) {
          this.searchParams.id = file.id
          this.getFileList()
          if (!this.searchParams.fileName) {
            this.breadcrumb.push(file)
    ...
    onHistroy() {
      ...
      this.historyRef.projectId = this.case.projectId
