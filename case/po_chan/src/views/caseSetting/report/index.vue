...
<template>
  ...
  <div class="h-full-15 px-5 pb-5 bg-white">
    <!--文档列表-->
    ...
    <Table :data="tableData" :columns="tableHeader" @fetchData="getFileAll" @sizeChange="tableData.pageSize = $event; getFileAll()"  >
      <template #caozuo="scope" >
          <div class="flex gap-2" v-if="buttonAuth[EXPORT_BUTTON]">
            <span
...
import { DELETE_BUTTON, IMPORT_BUTTON, EXPORT_BUTTON } from '@/constant/index.js'
export default {
  setup() {
    const appStore = useAppStore()
    const { caseInfo } = storeToRefs(useCaseStore())
    const { buttonAuth } = useButtonAuth([DELETE_BUTTON, IMPORT_BUTTON, EXPORT_BUTTON], caseInfo.value.isEdit)
    return { appStore, caseInfo, buttonAuth, DELETE_BUTTON, IMPORT_BUTTON, EXPORT_BUTTON }
  },
  data() {
    return {
      treeStep: {},
  ...
  mounted() {
    this.projectId = this.caseInfo.projectId
    this.companyId = this.caseInfo.companyId
    this.status = this.caseInfo.status
    this.isEdit = this.caseInfo.isEdit
    this.getFileAll()
  },
