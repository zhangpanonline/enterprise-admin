  ...
  try {
    ...
    if (res.success) {
      ...
      if (res.data.startDate) {
        time.value[0] = res.data.startDate
        time.value[1] = res.data.endDate
      }
    } else {
...
function onSave () {
  ...
  formRef.value.validate(async bool => {
    ...
    try {
      ...
      if (time.value) {
        form.startDate = time.value[0]
        form.endDate = time.value[1]
      }
      ...
      const res = await api({ type: MAIN_TYPE, bankruptAssetsOtherParam: form })
      if (res.success) {
