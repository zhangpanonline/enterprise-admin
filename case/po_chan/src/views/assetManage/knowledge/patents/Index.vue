<template>
  <el-header class="flex  items-center justify-between" >
    ...
    <el-input v-model="search.assetsName" placeholder="请输入专利名称" class="cus_search" clearable @change="fetchData()"></el-input>
    <el-input v-model="search.assetsNo" placeholder="请输入注册号" class="cus_search" clearable @change="fetchData()"></el-input>
    <el-select v-model="search.assetsStatus" placeholder="请选择状态" clearable class="cus_search" @change="fetchData()">
...
import { TAB2_CREATE, TAB2_EDIT, TAB2_DELETE, TAB2_EXPORT } from '@/constant/index.js'
const { caseInfo } = storeToRefs(useCaseStore())
const { buttonAuth } = useButtonAuth([TAB2_CREATE, TAB2_EDIT, TAB2_DELETE, TAB2_EXPORT], caseInfo.value.isEdit)
const { knowledge_patents_assets_status: assetsStatusDict } = useDict('knowledge_patents_assets_status')
...
const exportAll = async(type) => {
  ...
  downloadElement.download = '专利列表' // 下载后文件名
