<template>
  <el-header class="flex  items-center justify-between" >
    ...
    <el-input v-model="search.assetsName" placeholder="请输入软著名称" class="cus_search" clearable @change="fetchData()"></el-input>
...
import { TAB3_CREATE, TAB3_EDIT, TAB3_DELETE, TAB3_EXPORT } from '@/constant/index.js'
const { caseInfo } = storeToRefs(useCaseStore())
const { buttonAuth } = useButtonAuth([TAB3_CREATE, TAB3_EDIT, TAB3_DELETE, TAB3_EXPORT], caseInfo.value.isEdit)
const { knowledge_soft_assets_status: assetsStatusDict } = useDict('knowledge_soft_assets_status')
...
const exportAll = async(type) => {
  ...
  downloadElement.download = '软著列表' // 下载后文件名
