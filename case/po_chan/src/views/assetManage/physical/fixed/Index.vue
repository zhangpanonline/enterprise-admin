   ...
   <div class="bg-white px-5 h-15 flex items-center justify-between">
      <div class="flex gap-5 justify-start items-center" >
        ...
        <div class="flex gap-5" >
          <el-button v-if="buttonAuth[TAB1_IMPORT]" class="!m-0" type="primary"  @click="exportTemplate">
          ...
          <el-upload
            ...
            >
            <template #trigger>
              <el-button v-if="buttonAuth[TAB1_IMPORT]" class="!m-0" type="primary" >
                <template #icon><IconEpDownload /></template>
                导入
            </el-button>
            </template>
          ...
          <el-button v-if="buttonAuth[TAB1_EXPORT]" type="primary" @click="exportAll(true)">
            <template #icon><IconEpDownload /></template>
            选择导出
          </el-button>
...
import { TAB1_CREATE, TAB1_EDIT, TAB1_DELETE, TAB1_EXPORT, TAB1_IMPORT } from '@/constant/index.js'
const { caseInfo } = storeToRefs(useCaseStore())
const { buttonAuth } = useButtonAuth([TAB1_CREATE, TAB1_EDIT, TAB1_DELETE, TAB1_EXPORT, TAB1_IMPORT], caseInfo.value.isEdit)
...
const exportTemplate = async() => {
  ...
  downloadElement.download = '固定资产模版' // 下载后文件名
...
const exportAll = async(type) => {
  ...
  downloadElement.style.display = 'none'
  downloadElement.href = href
  downloadElement.download = '固定资产列表' // 下载后文件名
  document.body.appendChild(downloadElement)
  downloadElement.click() // 点击下载
  document.body.removeChild(downloadElement) // 下载完成移除元素
  window.URL.revokeObjectURL(href) // 释放掉blob对象
