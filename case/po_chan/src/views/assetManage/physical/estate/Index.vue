<template>
  <el-header class="flex  items-center justify-between" >
    ...
    <el-input v-model="search.assetsNo" placeholder="请输入编号" class="cus_search" clearable @change="fetchData()"></el-input>
    <el-select v-model="search.assetsBrand" placeholder="请选择权利类型" clearable class="cus_search" @change="fetchData()">
    ...
    <div class="flex gap-5" >
      <el-button v-if="buttonAuth[TAB3_IMPORT]" class="!m-0" type="primary"  @click="exportTemplate">
      ...
      <el-upload
        ...
        action="string">
        <template #trigger>
          <el-button v-if="buttonAuth[TAB3_IMPORT]" class="!m-0" type="primary" >
            <template #icon><IconEpDownload /></template>
            导入
          </el-button>
        </template>
      </el-upload>
      <el-button v-if="buttonAuth[TAB3_EXPORT]" type="primary" @click="exportAll(true)">
        <template #icon><IconEpDownload /></template>
        选择导出
      </el-button>
      <el-button v-if="buttonAuth[TAB3_EXPORT]" class="!m-0" type="primary" @click="exportAll(false)">
        <template #icon><IconEpDownload /></template>
        全部导出
      </el-button>
      <el-button v-if="buttonAuth[TAB3_CREATE]" class="!m-0" type="primary" @click="onEdit()">
    ...
    <Table :data="tableData" :columns="tableHeader" @selection-change="addComList" @fetchData="fetchData" @sizeChange="tableData.pageSize = $event; fetchData()"  >
      ...
      <template #caozuo="scope">
        <div class="flex gap-3">
          ...
          <span v-if="buttonAuth[TAB3_EDIT]" class="text-primary cursor-pointer"  @click="onEdit(scope.row)"> 编辑</span>
          <span v-if="buttonAuth[TAB3_DELETE]" class="text-warning cursor-pointer"  @click="onDel(scope.row)"> 删除</span>
        </div>
      </template>
    </Table>
  </div>
...
import { TAB3_CREATE, TAB3_EDIT, TAB3_DELETE, TAB3_EXPORT, TAB3_IMPORT } from '@/constant/index.js'
const { caseInfo } = storeToRefs(useCaseStore())
const { buttonAuth } = useButtonAuth([TAB3_CREATE, TAB3_EDIT, TAB3_DELETE, TAB3_EXPORT, TAB3_IMPORT], caseInfo.value.isEdit)
const { formatCNY } = useFormatMoney()

const { physical_estate_assets_brand: assetsStatusDict } = useDict('physical_estate_assets_brand')
...
const exportTemplate = async() => {
  ...
  downloadElement.download = '不动产模版' // 下载后文件名
...
const exportAll = async(type) => {
  ...
  downloadElement.style.display = 'none'
  downloadElement.href = href
  downloadElement.download = '不动产列表' // 下载后文件名
  document.body.appendChild(downloadElement)
  downloadElement.click() // 点击下载
  document.body.removeChild(downloadElement) // 下载完成移除元素
  window.URL.revokeObjectURL(href) // 释放掉blob对象
