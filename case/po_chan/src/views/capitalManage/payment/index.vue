<template>
  ...
  <div class="bg-white px-5 h-15 flex items-center justify-between" >
    <div class="flex gap-5 justify-start items-center" >
      ...
      <el-date-picker type="daterange" v-model="time" clearable start-placeholder="开始时间" end-placeholder="结束时间" value-format="YYYY-MM-DD" @change="fetchData()" ></el-date-picker>
  ...
  <div class="h-full-29 bg-white px-5 overflow-hidden relative">
    <Table :data="tableData" :columns="tableHeader"   @selection-change="addComList" @fetchData="fetchData" @sizeChange="tableData.pageSize = $event; fetchData()"  >
      <template #applyAmount="scope">
        {{ formatCNY(scope.row.applyAmount) }}
      </template>
    ...
    <p class="absolute bottom-5 left-5 text-sm text-black-300" >申请付款总金额：{{formatCNY(applyAmount) }}</p>
  </div>
