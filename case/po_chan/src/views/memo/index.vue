<template>
  ...
  <div class="bg-white h-full-14 px-5 box-border">
    <div class="h-15 flex items-center gap-5">
      <el-date-picker
      ...
      <el-button type="primary" @click="getFileAll(1)">
...
export default {
  ...
  setup() {
    ...
    return { appStore, caseInfo, buttonAuth, CREATE_BUTTON, EDIT_BUTTON, DELETE_BUTTON }
  },
  data() {
    return {
      searchData: {
  ...
  methods: {
    ...
    async getFileAll(pageNum = 1) {
      try {
        this.searchData.projectId = this.caseInfo.projectId
        this.searchData.pageNum = pageNum
        this.tableData.loading = true
        this.tableData.pageNum = pageNum
        this.searchData.pageSize = this.tableData.pageSize
        const response = await getMemoListApi(this.searchData)
        if (response.success) {
    ...
    async saveFileDate() {
      this.ruleForm.projectId = this.caseInfo.projectId
