...
export const request = (config?: IRequestConfig): any => {
  ...
  instance.interceptors.request.use(
    (config: IRequestConfig): any => {
      ...
      // 设置token
      if (token) config.headers![tokenKey] = `Bearer ${token}`;
      const { loading = showLoading } = config;
      if (loading) {
        if (requestNum > 0) removeLoading();
        addLoading();
      }
      // const requestTokenKey = `${config.method}_${config.url}_${JSON.stringify(config.params)}_${JSON.stringify(config.data)}`;
      // const cancelToken = debounceTokenCancel.get(requestTokenKey);
      // if (cancelToken) {
      //   console.log('取消请求', requestTokenKey);
      //   cancelToken();
      // }
  ...
  if (status) {
    switch (status) {
      ...
      case 401:
        ...
        userStore.clearUser();
        uni.redirectTo({ url: '/sub-pages/login/index' });
        break;
      case 403:
