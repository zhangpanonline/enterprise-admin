<template>
	<view class="wu-calendar" @click.stop>
		...
		<view v-if="insert || show" class="wu-calendar__content"
			...
			<slot name="header" :nowDate="nowDate">
				<view class="wu-calendar__header">
					...
					<template v-else>
						...
						<!-- <picker mode="date" :value="nowDate.fullDate" fields="month" @change="bindDateChange"> -->
							<text 
								v-if="customType === 'month'"
								class="wu-calendar__header-text">{{ (nowDate.year||'') + YearText }}</text>
								<text v-else
								class="wu-calendar__header-text">{{ (nowDate.year||'') + YearText + ( nowDate.month||'') + MonthText }}</text>
						<!-- </picker> -->
			...
			<view v-else class="wu-calendar__box">
				<view v-if="customType === 'day'" class="wu-calendar__weeks">
				...
				<swiper v-if="slideSwitchMode !== 'none'" :class="{'wu-calendar__weeks_container': initStatus}"
					...
					<template v-if="type === 'month' || type === 'week'">
						<swiper-item>
							<wu-calendar-block :weekList="weekList" :monthList="monthList" :selectedWeek="selectedWeek" :selectedMonth="selectedMonth" :weeks="preWeeks" :customType="customType" :calendar="calendar" :selected="selected"
						...
						<swiper-item>
								<wu-calendar-block :weekList="weekList" :monthList="monthList" :selectedWeek="selectedWeek" :selectedMonth="selectedMonth" :weeks="weeks" :customType="customType" :calendar="calendar" :selected="selected" :lunar="lunar"
						...
						<swiper-item>
							<wu-calendar-block :weekList="weekList" :monthList="monthList" :selectedWeek="selectedWeek" :selectedMonth="selectedMonth" :weeks="nextWeeks" :customType="customType" :calendar="calendar" :selected="selected"
								:lunar="lunar" @change="choiceDate" :color="color" :actBadgeColor="actBadgeColor" :startText="startText"
...
<script>
	...
	export default {
		...
		methods: {
			 ...
			 */
			choiceDate(weeks) {
				if (['week', 'month'].includes(this.customType)) {
			...
			updateWeeks(isChange = true, isInt = false) {
				...
				if (isChange) {
					...
					if (this.customType === 'month') {
						newFullDate = this.cale.getDate(fullDate, this.swiperChangeDirection === 'next' ? +1 : -1, 'year').fullDate
					}
				} else {
