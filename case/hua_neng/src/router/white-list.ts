...
const whiteListByPath: string[] = [
  ...
  '/sub-pages/register/index',
  '/sub-pages/register/result',
  '/sub-pages/serviceProtocol/index',
  '/sub-pages/privacyProtocol/index',
];
...
export const isWhiteList = (to: any) => {
  if (JSON.parse(VITE_OPNE_NO_LOGIN))
    return whiteListByPath.indexOf(to.replace(/\?.*/, '')) !== -1;
  else {
