...
const onSubmit = async () => {
  const res = await uni.request({
    url: `${getApp().globalData.base_url}/rest/v1/records?apikey=${getApp().globalData.apikey}`,
    dataType: "json",
    method: "POST",
    header: {
...
async function getCategoryTree() {
  try {
    const res = await uni.request({
      url: `${getApp().globalData.base_url}/rest/v1/rpc/get_category_tree?apikey=${getApp().globalData.apikey}`,
      dataType: "json",
    });
