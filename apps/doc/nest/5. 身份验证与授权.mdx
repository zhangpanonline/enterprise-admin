...
### 2.1 基本概念
Passport 是最流行的 Node.js 身份验证中间件，广泛应用于许多生产应用中。它支持多种身份验证策略，包括本地认证（用户名和密码）、OAuth 策略、OAuth2 策略、OpenID Connect 策略、JWT 策略等，可以满足不同场景下的身份认证需求。  

Passport 基于策略模式扩展不同的验证方式，它不直接提供 JWT 的具体实现，而是扩展了一个名为 passport-jwt 的策略包，用于在 Node.js 应用中实现基于 JWT 的无状态身份验证机制。JWT 被用作验证令牌（Token），在客户端与服务端之间传递用户身份信息，而 Passport 负责验证和解析 JWT 中的 Payload，将用户信息添加到请求对象中，以便在后续的路由处理程序中通过请求对象来获取用户信息，进行访问控制和权限判断。

需要注意的是，在 Nest 中依旧可以单独用 JWT进行身份认证，使用 Passport 是因为它提供了进一步的抽象，以简化身份认证流程。此外， Passport 与 Nest 框架的分层理念相契合，这使得在开发大型应用时，身份认证的维护和扩展变得更加容易。
### 2.2 本地策略实现用户登录
1. 安装依赖包
```bash
...
```
需要继承 PassportStrategy 并传入 passport-local 提供的 Strategy 类。
通过 super 方法可以配置用户名和密码的字段名称，默认是 username 和 password。 
在 validate 方法中调用 AuthService 的 validateUser 方法验证用户，
    如果验证失败则抛出 UnauthorizedException 异常，
    验证成功则返回用户信息，该信息会被附加到请求对象的 user 属性上。
3. 创建 AuthService 的 validateUser 方法
```typescript
...
```
4. 在 AuthModule 中注册 LocalStrategy
```typescript
...
```
4. 在 AuthModule 中注册 JwtStrategy 和 JwtModule
```typescript
import { Module } from '@nestjs/common';
import { PassportModule } from '@nestjs/passport';
...
### 2.4 总结
通过结合使用 Passport 和 JWT，可以在 Nest 应用中实现强大且灵活的身份验证机制。
Passport 提供了多种身份验证策略，简化了身份验证流程，而 JWT 则实现了无状态的身份验证，适合分布式系统和微服务架构。
通过这种方式，可以确保应用的安全性，同时提升用户体验。
